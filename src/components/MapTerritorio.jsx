    ...
    if (data && Array.isArray(data) && data.length > 0) {
      try {
        const features = data
          .map(
            (item) =>
              item.geo_dis_json || item.geo_pro_json || item.geo_dep_json
          )
          .filter(Boolean)
    ...
    const fetchTerritorio = async () => {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/api/mantenimiento/siam/territorio`
        );
        if (!response.ok)
          throw new Error("Error al cargar datos del territorio");
        const data = await response.json();
        setRawData(Array.isArray(data) ? data : []);

        // Fetch Peru Boundary
        const boundaryRes = await fetch(
          `${API_BASE_URL}//api/mantenimiento/siam/peru-boundary`
        );
        if (boundaryRes.ok) {
              ...
              <LayerGroup>
                {provincias.map((p, i) => (
                  ...
                  >
                    <Tooltip sticky>
                      <div className="text-sm">
                        <strong>{p.props.nombre}</strong>
                        <br />
                        Provincia de: {p.props.parent}
                        <br />
              ...
              <LayerGroup>
                {distritos.map((d, i) => (
                  ...
                  >
                    <Tooltip sticky>
                      <div className="text-sm">
                        <strong>Territorio SIAMsoft</strong>
                        <br />
                        Municipalidad de {d.props.nombre}
                        <br />
                        Departamento: {d.props.parent_dep}
                        <br />
                        Provincia: {d.props.parent_prov}
                        <br />
                        Ubigeo: {d.props.codigo}
                      </div>
                    </Tooltip>
